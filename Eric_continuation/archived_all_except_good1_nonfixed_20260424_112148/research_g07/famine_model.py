...
# Midpoint scaling: scaled[i] = rice[i] * (2*G0/(rice[0]+rice[1])) so G0 = (scaled[0]+scaled[1])/2 (G0 between first two points).
RICE_0 = float(positive_rice_production[0])
RICE_1 = float(positive_rice_production[1])
RICE_SUM_01 = RICE_0 + RICE_1


def scaling_factor_from_G0(G0):
    return 2.0 * float(G0) / RICE_SUM_01


FIRST_POINT_TARGET = 0.5
FIRST_POINT_PEN_W = 120.0
SCALED_SERIES_MAX_PEN_W = 4.0e5
...
def save_checkpoint(p_log, p_lin, loss, mse_hist_eh, gap, penalty):
    ...
    try:
        plot_results(p_lin, plot_prefix, save_plot_n=False)
...
def crash_window_hunt_explains(p_lin):
    ...
    scaling_factor = scaling_factor_from_G0(G0)
...
def evaluate_params(p_log, track_best=True):
    ...
    try:
        ...
        s_first = float(historical_scaled[0])
        if s_first < FIRST_POINT_TARGET:
            penalty += FIRST_POINT_PEN_W * (FIRST_POINT_TARGET - s_first) ** 2
        mx_scaled = float(np.max(historical_scaled))
        if mx_scaled > 1.0:
            penalty += (mx_scaled - 1.0) * SCALED_SERIES_MAX_PEN_W
...
def plot_results(best_params, plot_name="plot", save_plot_n=True):
